...
export default function DriverSync() {
  ...
  const [queue, setQueue] = useState(() => {
    try { return JSON.parse(localStorage.getItem('ecotrace_sync_queue') || '[]'); } catch { return []; }
  });
  ...
  const addDemoItem = () => {
    ...
    setQueue(newQueue);
    localStorage.setItem('ecotrace_sync_queue', JSON.stringify(newQueue));
  };

  const syncAll = () => {
    const synced = queue.map(q => ({ ...q, status: 'synced' }));
    setQueue(synced);
    localStorage.setItem('ecotrace_sync_queue', JSON.stringify(synced));
  };

  const clearQueue = () => {
    setQueue([]);
    localStorage.removeItem('ecotrace_sync_queue');
  };
